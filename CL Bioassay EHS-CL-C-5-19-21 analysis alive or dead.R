...
CL.dat <- read.csv("Bioassays mortality - EHS-CL-C-6-15-21 alive or dead.csv")

CL.dat2 <-  CL.dat %>%
  select(Day, Group, Treatment, Dead, Alive) %>%
  pivot_longer(cols=c(-Day, -Treatment,-Group), names_to="Status", values_to = "Individuals") %>%
  mutate(Status=as.factor(Status)) %>%
  mutate(Status=factor(Status, levels=levels(Status)[c(2,1)])) %>%
  drop_na(Individuals)
...
cl.dat.survival <- CL.dat %>%  
  select(Day, Treatment, Group, Alive, Dead ) %>%  
  group_by(Day, Group, Treatment) %>%
  summarise(survival=((Alive/(Alive+Dead))*100))

cl.dat.survival.reps <- cl.dat.survival %>%
group_by( Day, Treatment) %>%
  summarize(Percent_survival = mean(survival))

ggplot(cl.dat.survival.reps, aes(x=Day, y=Percent_survival , fill=paste(Treatment, sep="-")))+geom_col(position="dodge")+   scale_fill_manual(values=palette1)+ theme+labs(fill='Treatment') 

ggplot(cl.dat.survival.reps, aes(x=Day, y=Percent_survival , color=Treatment))+
  scale_color_manual(values=palette1)+ theme+ geom_line()
...
group1 = CL.dat2[CL.dat2$Day==6 & CL.dat2$Treatment=="CL" & CL.dat2$Status=="Dead",]$Individuals
group2 = CL.dat2[CL.dat2$Day==6 & CL.dat2$Treatment=="No fungus" & CL.dat2$Status=="Dead",]$Individuals
